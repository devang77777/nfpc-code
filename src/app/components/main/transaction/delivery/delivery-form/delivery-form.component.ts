...
export class DeliveryFormComponent implements OnInit, OnDestroy {
  ...
  mapItem(apiItem) {
    const newItem: OrderItemsPayload = {
      ...
      item_update: apiItem && apiItem.item_update ? Number(apiItem.item_update) : 0,
      ...
      original_item_uom_id: apiItem && apiItem.original_item_uom_id ? apiItem.original_item_uom_id : null
    };
    return newItem;
  }
    ...
    if (item) {
      itemControls.push(
        this.formBuilder.group({
          item: new FormControl(
            { id: item?.item.id, item_code: item?.item.item_code, name: item?.item.name },
            [Validators.required]
          ),
      ...
      itemControls.push(
        this.formBuilder.group({
          ...
          reason_id: new FormControl({ value: null, disabled: true }, [Validators.required]),
          original_item_qty: new FormControl(0, [Validators.required]),
          is_picking: new FormControl(item?.is_picking == 1 ? true : false, [Validators.required]),
          original_item_uom_id: new FormControl(null),
  ...
  setUpRelatedUom(selectedItem: any, formGroup: FormGroup, isEdit?: boolean) {
    ...
    if (selectedItem.item_main_price && selectedItem.item_main_price.length > 0) {
  ...
  loadChangesData(result, itemControls, newFormGroup) {
    ...
    if (
      ...
      if (body.item_qty) {
        if (this.isEditForm) {
          if (+result.item_update > +body.item_qty || +result.original_item_qty == 0) {
            this.subscriptions.push(
          ...
          } else if (+result.item_update < +body.item_qty) {
  ...
  changeQty(currentQty, originalItemQty, i) {
    if (+originalItemQty != 0) {
      ...
      if (+currentQty < +originalItemQty) {
        ...
        ((this.orderFormGroup.get('items') as FormArray).at(i) as FormGroup).controls['reason_id'].disable();
      }
    }
    this.reason_index = i;
  }
  ...
  fiterItems(value) {
    ...
    if (value !== '') {
      ...
      this.filteredItems = this.items.filter(x => x.item.item_code.toLowerCase().trim() === this.itemfilterValue || x.item.item_name.toLowerCase().trim() === this.itemfilterValue);
  ...
  fiterReason(event) {
    ...
    if (value !== '') {
      this.reasonsList = this.filterReason.filter(i =>
        i.name.toLowerCase()?.trim().includes(value.toLowerCase().trim()) || i.code.toLowerCase()?.trim().includes(value.toLowerCase().trim()));
    } else {
  ...
  public postFinalOrder(target: string): void {
    ...
    finalPayload.items.map(i => {
      this.deliveryData.delivery_details.forEach(element => {
        i.is_deleted = element.is_deleted ? element.is_deleted : null;
        // i.is_picking = element.is_picking === false ? 0 : 1
        if (element.item.id === i.item.id) {
          i['is_changed'] = 0;
          i['id'] = element.id;
          i.reason_id = element?.reason?.id ? element?.reason?.id : null;
      ...
      if (i.promotion_id == undefined) {
        i.promotion_id = null;
      }
    });
    body.items.forEach(element => {
      finalPayload.items.forEach(dd => {
        if (dd.item.id == element.item.id) {
          dd.is_picking = element.is_picking === true ? 1 : 0;
          if (!dd.reason_id) {
            dd.reason_id = element?.reason_id?.id ? element?.reason_id?.id : null
          }
        }
      });
    });
    ...
    this.finalOrderPayload.items.forEach((item, index) => {
      ...
      item.uuid = this.deliveryData.delivery_details[index]?.uuid
    });
    // this.finalOrderPayload['is_changed'] = 1;
