...
@Component({
  selector: 'app-caps-import',
  templateUrl: './caps-import.component.html',
  styleUrls: ['./caps-import.component.scss'],
  
})
export class CapsImportComponent implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['transaction/customer-caps']).then();
  }
  ...
  importData() {
    ...
    this.api.capsImport(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
