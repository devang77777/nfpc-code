...
@Component({
  ...
  styleUrls: ['./credit-note-update.component.scss']
})
export class CreditNoteUpdateComponent implements OnInit {
  ...
  importData() {
    ...
    formData.append('return_update_file', this.fileInfo);

    this.creditNoteService.updateCreditNote(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
