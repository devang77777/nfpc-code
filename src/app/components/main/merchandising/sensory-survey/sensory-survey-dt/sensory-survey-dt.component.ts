...
@Component({
  selector: 'app-sensory-survey-dt',
  templateUrl: './sensory-survey-dt.component.html',
  styleUrls: ['./sensory-survey-dt.component.scss']
})
export class SensorySurveyDtComponent implements OnInit {

  @Output() public itemClicked: EventEmitter<any> = new EventEmitter<any>();
  @Output() public selectedRows: EventEmitter<any> = new EventEmitter<any>();
  @Input() public isDetailVisible: boolean;
  @Input() public newSensorySurveyData: any = {};
  ...
  public ngOnInit(): void {
    ...
    this.subscriptions.push(this.eventService.on(Events.SEARCH_SENSORY, ({ request, requestOriginal, response }) => {
  ...
  onCloseCriteria() {
    this.advanceSearchRequest = []
    this.eventService.emit(new EmitEvent(Events.CHANGE_CRITERIA, { reset: true, module: Events.SEARCH_SENSORY, route: '/merchandising/sensory-survey' }));
  }
  onChangeCriteria() {
    this.eventService.emit(new EmitEvent(Events.CHANGE_CRITERIA, { route: '/merchandising/sensory-survey' }));
  }
  ...
  getSensorySurveyList() {
    ...
    this.subscriptions.push(this.merService.getSensorySurveyList(this.filterForm.value).subscribe((res: any) => {
      this.apiResponse = res;
      this.allResData = res.data;
      this.updateDataSource(res.data);
    }));
  ...
  ngOnChanges(changes: SimpleChanges) {
    //console.log(changes);
    if (changes) {
      if (changes.newSensorySurveyData && Object.keys(changes.newSensorySurveyData.currentValue).length > 0) {
        let currentValue = changes.newSensorySurveyData.currentValue;
        this.newSensorySurveyData = currentValue;
        this.updateAllData(this.newSensorySurveyData);
      }
    }
  }
  ...
  updateAllData(data) {
    this.subscriptions.push(
      this.merService.getSensorySurveyList(this.filterForm.value).subscribe((res) => {
        this.apiResponse = res;
        this.allResData = res.data;
        this.updateDataSource(res.data);
