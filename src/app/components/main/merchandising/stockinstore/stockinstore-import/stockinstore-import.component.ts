...
@Component({
  selector: 'app-stockinstore-import',
  templateUrl: './stockinstore-import.component.html',
  styleUrls: ['./stockinstore-import.component.scss'],
  providers: [
    {
...
export class StockinstoreImportComponent implements OnInit {
  public domain = environment.host
  ...
  ngOnInit(): void {
    this.importForm = new FormGroup({
      assigninventory_file: new FormControl(),
  ...
  backToMain() {
    this.router.navigate(['/merchandising/stockinstore']);
  }
  get getFieldFrom() {
  ...
  mappingfield() {
    this._service.getStockInStoreMappingfield().subscribe((res: any) => {
      if (res.status) {
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._service
      .submitFinalImportStockInStore({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
        this.commonToasterService.showSuccess(
          'Data has been imported successfully.'
        );
        this.router.navigateByUrl('/merchandising/stockinstore');
      });
  }

  importCustomer() {
    const formData = new FormData();
    formData.append('assigninventory_file', this.fileInfo);
    formData.append('skipduplicate', this.importFile.skipduplicate);
    formData.append('map_key_value', JSON.stringify(this.mapFields));

    this._service.importStockInStore(formData).subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.duplicateData = [];
        this.unMappedData = [];
