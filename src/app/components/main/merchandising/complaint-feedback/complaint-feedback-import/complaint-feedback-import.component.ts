...
@Component({
  selector: 'app-complaint-feedback-import',
  templateUrl: './complaint-feedback-import.component.html',
  styleUrls: ['./complaint-feedback-import.component.scss'],
  providers: [
    {
...
export class ComplaintFeedbackImportComponent implements OnInit {
  ...
  constructor(
    private _serivce: MerchandisingService,
    router: Router,
    private formBuilder: FormBuilder,

    private commonToasterService: CommonToasterService
  ) {
    Object.assign(this, { _serivce, router });
  }

  ngOnInit(): void {
    this.importForm = new FormGroup({
      complaint_file: new FormControl(),
  ...
  backToMain() {
    this.router.navigate(['/merchandising/complaint-feedback']);
  }
  get getFieldFrom() {
  ...
  remove(filesList): void {
    ...
    const file = document.querySelector('#complaint_file') as HTMLInputElement;
    if (file) {
      file.value = null;
    }
  }
  ...
  mappingfield() {
    this._serivce.getComplaintMappingfield().subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._serivce
      .submitImportComplaint({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
        this.commonToasterService.showSuccess(
          'Data has been imported successfully.'
        );
        this.router.navigateByUrl('/merchandising/complaint-feedback');
      });
  }

  importCustomer() {
    const formData = new FormData();
    formData.append('complaintfeedback_file', this.fileInfo);
    formData.append('skipduplicate', this.importFile.skipduplicate);
    formData.append('map_key_value', JSON.stringify(this.mapFields));

    this._serivce.importComplaintsFeedback(formData).subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.duplicateData = [];
        this.unMappedData = [];
