...
@Component({
  ...
  templateUrl: './asset-track-import.component.html',
  styleUrls: ['./asset-track-import.component.scss'],
  providers: [
    {
...
export class AssetTrackImportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.importForm = new FormGroup({
      assettracking_file: new FormControl(),
  ...
  backToMain() {
    this.router.navigate(['/merchandising/asset-tracking']);
  }
  get getFieldFrom() {
  ...
  mappingfield() {
    this._service.getAssetTrackingMappingfield().subscribe((res: any) => {
      if (res.status) {
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._service
      .submitImportAssetTracking({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
        this.commonToasterService.showSuccess(
          'Data has been imported successfully.'
        );
        this.router.navigateByUrl('/merchandising/asset-tracking');
      });
  }

  importCustomer() {
    const formData = new FormData();
    formData.append('assettracking_file', this.fileInfo);
    formData.append('skipduplicate', this.importFile.skipduplicate);
    formData.append('map_key_value', JSON.stringify(this.mapFields));

    this._service.importAssetTracking(formData).subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.duplicateData = [];
        this.unMappedData = [];
