...
@Component({
  selector: 'app-planogram-import',
  templateUrl: './planogram-import.component.html',
  styleUrls: ['./planogram-import.component.scss'],
})
export class PlanogramImportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.importForm = new FormGroup({
      planogram_file: new FormControl(),
  ...
  backToMain() {
    this.router.navigate(['/merchandising/planograms']);
  }
  get getFieldFrom() {
  ...
  remove(filesList): void {
    ...
    const file = document.querySelector('#planogram_file') as HTMLInputElement;
    if (file) {
      file.value = null;
    }
  }
  ...
  mappingfield() {
    this._service.getPlanogramMappingfield().subscribe((res: any) => {
      if (res.status) {
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._service
      .submitImportPlanogram({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
        this.commonToasterService.showSuccess(
          'Data has been imported successfully.'
        );
        this.router.navigateByUrl('/merchandising/planograms');
      });
  }

  importCustomer() {
    const formData = new FormData();
    formData.append('planogram_file', this.fileInfo);
    formData.append('skipduplicate', this.importFile.skipduplicate);
    formData.append('map_key_value', JSON.stringify(this.mapFields));

    this._service.importPlanogram(formData).subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.duplicateData = [];
        this.unMappedData = [];
