...
@Component({
  selector: 'app-competitor-import',
  templateUrl: './competitor-import.component.html',
  styleUrls: ['./competitor-import.component.scss'],
  providers: [
    {
...
export class CompetitorImportComponent implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['/merchandising/competitors']);
  }
  get getFieldFrom() {
  ...
  mappingfield() {
    this._service.getCompetitorInfoappingfield().subscribe((res: any) => {
      if (res.status) {
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._service
      .submitImportCompetitor({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
        this.commonToasterService.showSuccess(
          'Data has been imported successfully.'
        );
        this.router.navigateByUrl('/merchandising/competitors');
      });
  }

  importCustomer() {
    const formData = new FormData();
    formData.append('competitorinfo_file', this.fileInfo);
    formData.append('skipduplicate', this.importFile.skipduplicate);
    formData.append('map_key_value', JSON.stringify(this.mapFields));

    this._service.importCompetitor(formData).subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.duplicateData = [];
        this.unMappedData = [];
