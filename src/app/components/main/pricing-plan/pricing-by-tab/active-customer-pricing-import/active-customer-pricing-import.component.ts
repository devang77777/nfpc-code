...
type AOA = any[][];
@Component({
  selector: 'app-active-customer-pricing-import',
  templateUrl: './active-customer-pricing-import.component.html',
  styleUrls: ['./active-customer-pricing-import.component.scss']
})
export class ActiveCustomerPricingImportComponent implements OnInit {
 @ViewChild('file') fileInput!: ElementRef<HTMLInputElement>;
  ...
  // // Sync model when form value changes
  // this.exportForm.get('type')?.valueChanges.subscribe(value => {
  //   this.export.type = value;
  // });
  // this.exportForm.get('spci_price')?.valueChanges.subscribe(value => {
  ...
  exportPricing(type) {
    const file_type = 'csv';
   const scopeMap  = {  '1': 'specific_pricing' } as const;
