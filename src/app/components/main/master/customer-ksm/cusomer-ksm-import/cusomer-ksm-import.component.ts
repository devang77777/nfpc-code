...
type AOA = any[][];
@Component({
  selector: 'app-cusomer-ksm-import',
  templateUrl: './cusomer-ksm-import.component.html',
  styleUrls: ['./cusomer-ksm-import.component.scss']
})
export class CusomerKsmImportComponent implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['masters/customer-ksm-mapping']).then();
  }
  ...
  importData() {
    ...
    formData.append('customer_kam_ksm', this.fileInfo);

    this.api.importCustomerKSMMapping(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
