...
@Component({
  selector: 'app-customer-supervisor-import',
  templateUrl: './customer-supervisor-import.component.html',
  styleUrls: ['./customer-supervisor-import.component.scss']
})
export class CustomerSupervisorImportComponent implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['masters/customer-supervisor']).then();
  }
  ...
  importData() {
    ...
    this.api.importCustomerSupervisor(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
