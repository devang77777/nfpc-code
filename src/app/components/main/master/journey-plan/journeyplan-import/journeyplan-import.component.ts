...
@Component({
  selector: 'app-journeyplan-import',
  templateUrl: './journeyplan-import.component.html',
  styleUrls: ['./journeyplan-import.component.scss'],
  providers: [{
    provide: STEPPER_GLOBAL_OPTIONS, useValue: { displayDefaultIndicatorType: false }
  }]
})
export class JourneyplanImportComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.importForm = new FormGroup({
      journeyplan_file: new FormControl(),
  ...
  mappingfield() {
    this._service.journeyPlanMappingfield().subscribe((res: any) => {
      if (res.status) {
        this.isActive = false;
        this.data = res.data;
  ...
  finalImport() {
    this._service
      .submitImportJourneyPlan({ successfileids: this.successfields, skipduplicate: this.skipduplicate })
      .subscribe((res: any) => {
        this.commonToasterService.showSuccess(
          'Data has been imported successfully.'
        );
        this.router.navigateByUrl('/masters/journey-plan');
      });
  }

  importCustomer() {
    const formData = new FormData();
    formData.append('journeyplan_file', this.fileInfo);
    formData.append('skipduplicate', this.importFile.skipduplicate);
    formData.append('map_key_value', JSON.stringify(this.mapFields));

    this._service.importJourneyPlan(formData).subscribe((res: any) => {
      if (res.status) {
        //console.log(res);
        this.duplicateData = [];
        this.unMappedData = [];
