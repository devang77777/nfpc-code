    ...
    this.subscriptions.push(
      this.dataEditor.newData.subscribe((result) => {
        ...
        if (data && data.uuid && this.isEdit) {
          let merchandiser = [];
          // data?.customer_merchandisers?.forEach(element => {
          //   merchandiser.push({ id: element?.merchandizer?.id, itemName: `${element?.merchandizer?.firstname} ${element?.merchandizer?.lastname}` });
          // });
          ...
          this.merchandiserFormControl.setValue(
            data.merchandiser_id ? data.merchandiser_id : ''
          );
  ...
  private postCustomerData(): void {
    ...
    this.apiService
      .addCustomers({
        ...
        merchandiser_id: this.merchandiserFormControl.value || 0,
