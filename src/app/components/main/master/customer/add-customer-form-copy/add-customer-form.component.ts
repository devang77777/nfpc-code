...
export class AddCustomerFormComponent extends BaseComponent implements OnInit {
  ...
  public trnNoFormControl: FormControl;
  ...
  customerLobList = [];
  customerLobTypeFormControl: FormControl;
  ...
  buildForm() {
    ...
    this.homeAddressFormControl = new FormControl('');
    ...
    this.trnNoFormControl = new FormControl('');
    ...
    this.customerLobTypeFormControl = new FormControl('');
    ...
    this.customerFormGroup = this.createCustomerForm();
  }


  createCustomerForm(): FormGroup {
    return new FormGroup({
      customerLobType: new FormControl(''),
      ...
      merchandiserId: new FormControl(''),
      ...
      salesOrganizationId: new FormControl(''),
  ...
  private postCustomerData(): void {
    ...
    this.apiService
      .addCustomers({
        ...
        trn_no: this.trnNoFormControl.value,
  ...
  onSelectLob(lob) {
    let formArray = this.getLobCustomerInfo;
    if (lob.length != 0) {
      lob.forEach((item,index) => {
        let filterLob = formArray.value.filter((x) => x.lobName == item.name);
        if (filterLob.length == 0) {
          formArray.push(this.createCustomerForm());
          formArray.controls[index].get('shiptoParty').setValue(this.customerFormGroup.get('customerCode').value);
          formArray.controls[index].get('soldtoParty').setValue(this.customerFormGroup.get('customerCode').value);
          formArray.controls[index].get('payer').setValue(this.customerFormGroup.get('customerCode').value);
          formArray.controls[index].get('billtoparty').setValue(this.customerFormGroup.get('customerCode').value);
          formArray.controls[index].get('lobName').setValue(item.name);
          formArray.controls[index].get('lobId').setValue(item.id);
