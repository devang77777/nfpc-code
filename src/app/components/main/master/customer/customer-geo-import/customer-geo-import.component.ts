...
type AOA = any[][];
@Component({
  selector: 'app-customer-geo-import',
  templateUrl: './customer-geo-import.component.html',
  styleUrls: ['./customer-geo-import.component.scss']
})
export class CustomerGeoImport implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['masters/customer']).then();
  }
  ...
  importData() {
    ...
    this.api.uploadPricingGeoReport(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
