...
type AOA = any[][];
@Component({
  selector: 'app-customer-zone-import',
  templateUrl: './customer-zone-import.component.html',
  styleUrls: ['./customer-zone-import.component.scss']
})
export class CustomerZoneImportComponent implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['masters/customer-region']).then();
  }
  ...
  importData() {
    ...
    formData.append('customer_region', this.fileInfo);

    this.api.importCustomerRegion(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
