...
type AOA = any[][];
@Component({
  selector: 'app-customer-branch-plant-import',
  templateUrl: './customer-branch-plant-import.component.html',
  styleUrls: ['./customer-branch-plant-import.component.scss']
})
export class CustomerBranchPlantImportComponent implements OnInit {
  ...
  backToMain() {
    this.router.navigate(['masters/customer-branch-plant']).then();
  }
  ...
  importData() {
    ...
    this.api.importCustomerBranchPlant(formData).subscribe((res: any) => {
      if (res.status) {
        this.backToMain();
