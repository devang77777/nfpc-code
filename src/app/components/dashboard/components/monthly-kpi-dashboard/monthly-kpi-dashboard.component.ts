...
@Component({
  selector: 'app-monthly-kpi-dashboard',
  templateUrl: './monthly-kpi-dashboard.component.html',
  styleUrls: ['./monthly-kpi-dashboard.component.scss']
})
export class MonthlyKpiDashboardComponent implements OnInit, AfterViewInit {
...
this.filterForm.value.zone_id.map((ids:any,index:any)=>{
  ...
  applyFilterOnClick() {
    ...
    let filterObj:any = {
      ...
      region_id:form?.zone_id.map((ids:any)=>ids.id).length>0?form?.zone_id.map((ids:any)=>ids.id):this.zoneList.map((ids:any)=>ids.id)
    };
  ...
  getData(filterObj) {
   ...
   this.apiService.getChartMontlyKpi(filterObj).subscribe((data:any)=>{
