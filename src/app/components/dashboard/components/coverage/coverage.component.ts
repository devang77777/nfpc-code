...
export class CoverageComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.subscriptions.push(
      this.service.getDashboard4FiltersData(body).subscribe((dahsboard) => {
        ...
        this.dashboardData['coverage'] = dahsboard.dashboardData.data;
        this.sortCoverageData();
        this.dashboardData['coverage'].details.reverse();
  ...
  getData(filterObj) {
    ...
    this.service.getDashboard4DataByFilter(filterObj).subscribe((res) => {
      this.dashboardData['coverage'] = res.data;
      this.sortCoverageData();
      this.dashboardData['coverage'].details.reverse();
  ...
  sortCoverageData() {
    this.dashboardData['coverage'].details = this.dashboardData['coverage'].details.sort((a, b) => {
  ...
  dataShowOrNot(){
    if(this.dashboardData['coverage'].length > 0 || Object.keys(this.dashboardData['coverage']).length > 0){
      return true;
    }
    else{
      return false;
