...
@Component({
  selector: 'app-visit-frequency',
  templateUrl: './visit-frequency.component.html',
  styleUrls: ['./visit-frequency.component.scss']
})
export class VisitFrequencyComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.filterControl = new FormControl('visitFrequency');
    ...
    body.type = 'visitFrequency';
    body.main_type = 'list';
    this.subscriptions.push(
      this.service.getDashboard4FiltersData(body).subscribe((dahsboard) => {

        this.dashboardData['visitFrequency'] = dahsboard.dashboardData.data;
        this.sortCoverageData();
        this.dashboardData['visitFrequency'].details.reverse();
        this.getChartsData('visitFrequency');
      })
    )
    // })
    this.subscriptions.push(
  ...
  getData(filterObj) {
    ...
    this.service.getDashboard4DataByFilter(filterObj).subscribe((res) => {
      this.dashboardData['visitFrequency'] = res.data;
      this.sortCoverageData();
      this.dashboardData['visitFrequency'].details.reverse();
  ...
  sortCoverageData() {
    this.dashboardData['visitFrequency'].details = this.dashboardData['visitFrequency'].details.sort((a, b) => {
      return Number(a.EXECUTION) - Number(b.EXECUTION);
    });
  }
  ...
  dataShowOrNot(){
    if(this.dashboardData['visitFrequency'].length > 0 || Object.keys(this.dashboardData['visitFrequency']).length > 0){
      return true;
    }
    else{
      return false;
